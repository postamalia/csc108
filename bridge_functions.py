...
def find_bridges_in_radius(bridges: list[list], lat: float, lon: float,
    ...
    for bridge in bridges:
        if ((bridge[COLUMN_ID] not in exclusions) and (calculate_distance
        (lat, lon, bridge[COLUMN_LAT], bridge[COLUMN_LON]) <= radius)):
            bridges_in_radius.append(bridge[COLUMN_ID])
...
def calculate_average_condition(bridge: list, start: int, stop: int) -> float:
    ...
    bci_scores = bridge[COLUMN_BCI][INDEX_BCI_SCORES]
    parallel_years = bridge[COLUMN_BCI][INDEX_BCI_YEARS]
    accumulated_scores = []
    total = 0.0
    ...
    i = 0
    while i < len(parallel_years):
        year = parallel_years[i]
        if stop > int(year) and start <= int(year):
            if bci_scores[i] != MISSING_BCI:
                accumulated_scores.append(bci_scores[i])
        i += 1

    if accumulated_scores == []:
        return total
    
    for score in accumulated_scores:
        total += score
    return total / len(accumulated_scores)
...
def inspect_bridge(bridges: list[list], bridge_id: int, inspect_date: str,
    ...
    bridge[COLUMN_BCI][INDEX_BCI_YEARS].insert(0, inspect_date[6:])
...
def find_worst_bci(bridges: list[list], bridge_ids: list[int]) -> int:
    ...
    for bridge_id in bridge_ids:
        bridge = find_bridge_by_id(bridges, bridge_id)
        last_bcis.append([(get_bridge_condition
        (bridges, bridge[COLUMN_ID])), bridge[COLUMN_ID]])
    
    last_bcis.sort()
    
    return min(last_bcis)[1]
...
def map_route(bridges: list[list], lat: float, lon: float,
    ...
    while (bridges_inspected < max_bridges) and (find_bridges_in_radius
    (bridges, lat, lon, radius, exclusions)):
        worst_bridge = find_worst_bci(bridges, (find_bridges_in_radius
        (bridges, lat, lon, radius, exclusions)))
        sequence.append(worst_bridge)
        exclusions.append(worst_bridge)
        lat = (find_bridge_by_id(bridges, worst_bridge))[COLUMN_LAT]
        lon = (find_bridge_by_id(bridges, worst_bridge))[COLUMN_LON]
        bridges_inspected += 1
...
def clean_span_data(raw_spans: str) -> list[float]:
    ...
    span_indexes = []  # This list will accumulate the index of the first 
    # digit of each span length and the index of each subsequent 
    # semicolon, in the same order as they appear in raw_spans
    
    span_index_pairs = []  # This list will accumulate the indexes in
    # span_indexes as lists of pairs
    
    span_lengths = []  # While the index pairs in span_index_pairs are 
    # iterated over in order to extract each span length from raw_spans 
    # as a slice, this list will accumulate the span lengths as floats
   
    i = raw_spans.index(' ')
    
    while i < len(raw_spans):
        if raw_spans[i] == '=':
            span_indexes.append((i + 1))
        elif raw_spans[i] == ';':
    ...
    while i < len(span_indexes):
        span_index_pairs.append([span_indexes[i], span_indexes[i + 1]])
        i += 2
    
    for pair in span_index_pairs:
        span_lengths.append(float(raw_spans[(pair[0]):(pair[1])]))
...
def clean_bci_data(bci_years: list[str], start_year: int, 
    ...
    for year in range(start_year, (start_year - (len(bci_scores))), -1):
        bci_years.append(str((year)))
